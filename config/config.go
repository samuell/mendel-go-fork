...
func (c *Config) validateAndAdjust() error {
	...
	if c.Basic.Pop_size%2 != 0 {
		return errors.New("basic.pop_size must be an even number")
	}
	if (c.Population.Num_linkage_subunits % c.Population.Haploid_chromosome_number) != 0 {
		return errors.New("num_linkage_subunits must be an exact multiple of haploid_chromosome_number")
	}

	c.Selection.Heritability = math.Max(1.e-20, c.Selection.Heritability) // Limit the minimum value of heritability to be 10**-20

	if c.Mutations.Max_fav_fitness_gain <= 0.0 {
		return errors.New("max_fav_fitness_gain must be > 0.0")
	}

	if c.Mutations.Allow_back_mutn && c.Computation.Tracking_threshold != 0.0 {
		return errors.New("can not set both allow_back_mutn and a non-zero tracking_threshold")
	}
	if c.Mutations.Multiplicative_weighting != 0.0 && c.Computation.Tracking_threshold != 0.0 {
		return errors.New("setting tracking_threshold with multiplicative_weighting is not yet supported")
	}
	...
	if c.Computation.Num_threads == 0 {
		c.Computation.Num_threads = uint32(runtime.NumCPU())
	}

	if c.Tribes.Num_tribes <= 0 {
		return errors.New("num_tribes can not be <= 0")
	}
...
func FindDefaultFile() string {
	...
	if CmdArgs.DefaultFile != "" {
		if _, err := os.Stat(CmdArgs.DefaultFile); err == nil {
			return CmdArgs.DefaultFile
		}
		log.Fatalf("Error: specified defaults file %v does not exist", CmdArgs.DefaultFile)
	}

	// Check for it in the current directory
	if _, err := os.Stat(DEFAULTS_INPUT_FILE); err == nil {
		return DEFAULTS_INPUT_FILE
	}
	lookedIn := []string{"."}
...
func Verbose(level uint32, msg string, args ...interface{}) {
	if Cfg.Computation.Verbosity >= level {
		log.Printf("V"+fmt.Sprint(level)+" "+msg, args...)
	}
}
