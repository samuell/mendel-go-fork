...
func ReadCmdArgs() {
	...
	if CmdArgs.InputFileToCreate != "" {
		if CmdArgs.InputFile != "" || useDefaults {
			log.Println("Error: if you specify -c you can not specify either -f or -d")
			Usage(1)
		}

	} else if useDefaults {
		if CmdArgs.InputFile != "" || CmdArgs.InputFileToCreate != "" {
			log.Println("Error: if you specify -d you can not specify either -f or -c")
			Usage(1)
		}
