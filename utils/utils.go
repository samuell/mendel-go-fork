...
func CopyFile(fromFile, toFile string) error {
	...
	if err := ioutil.WriteFile(toFile, buf, 0644); err != nil {
		return err
	}
	return nil
}
...
func CopyFromFileName2Writer(fromFileName string, toFile *os.File) error {
	...
	if _, err := io.WriteString(toFile, string(buf)); err != nil {
		return err
	}
	return nil
}
...
func CanonicalPathsEqual(filePath1, filePath2 string) (isEqual bool, err error) {
	...
	if cPath1, err = filepath.EvalSymlinks(filePath1); err != nil {
		return
	}
	if cPath1, err = filepath.Abs(cPath1); err != nil {
		return
	}
	if cPath2, err = filepath.EvalSymlinks(filePath2); err != nil {
		return
	}
	if cPath2, err = filepath.Abs(cPath2); err != nil {
		return
	}
...
func CreatePrefixedZip(pathToZipUp, zipFilePath, prefixToReplace, newPrefix string) error {
	...
	if newPrefix == "" {
		cleanPrefixToReplace = cleanPrefixToReplace + "/"
	}
	...
	err = filepath.Walk(pathToZipUp, func(filePath string, info os.FileInfo, err error) error {
		if err != nil {
			return err
		} // Walk() had a problem with this file
		if info.IsDir() {
			return nil
		} // SPC can't handle the dirs themselves in the zip
		if err != nil {
			return err
		}
		var zipFile string
