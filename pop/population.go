...
func PopulationFactory(prevPop *Population, genNum, tribeNum, partsPerPop uint32) *Population {
	...
	if genNum == 0 {
		...
		for i := 1; i <= cap(p.Parts); i++ {
			p.Parts = append(p.Parts, PopulationPartFactory(0, p))
		}
...
func GenerateAllUniqueInitialAlleles(p *Population, uniformRandom *rand.Rand) {
	if config.Cfg.Population.Num_contrasting_alleles == 0 || config.Cfg.Population.Initial_alleles_pop_frac <= 0.0 {
		return
	} // nothing to do
	...
	for i, indRef := range p.IndivRefs {
		...
		if i > 0 {
			ratioSoFar = float64(numWithAlleles) / float64(i)
		}
		// else ratioSoFar = 0
...
func ParseInitialAllelesFrequencies(frequencies string) (freqList []FractionFrequency) {
	...
	if strings.TrimSpace(frequencies) == "" {
		log.Fatal(errorStr)
	}
	...
	for _, p := range pairs {
		...
		if err != nil {
			log.Fatalf("Error: if initial_allele_fitness_model==%s, then initial_alleles_frequencies must be like: alfrac1:freq1, alfrac2:freq2, .... Parsing error: %v", string(VARIABLE_FREQ_INITIAL_ALLELES), err)
		}
		if alleleFraction <= 0.0 || alleleFraction > 1.0 {
			log.Fatalf("Error: initial_alleles_frequencies must contain allele fractions that are > 0.0 and <= 1.0. Bad allele fraction: %v", alleleFraction)
		}
		...
		if frequency <= 0.0 || frequency > 0.5 {
			log.Fatalf("Error: initial_alleles_frequencies must contain frequences that are > 0.0 and <= 0.5. Bad frequency: %v", frequency)
		}
		freqList = append(freqList, FractionFrequency{alleleFraction: alleleFraction, frequency: frequency})
		alleleTotal += alleleFraction
	}
	if alleleTotal != 1.0 {
		log.Fatalf("Error: the allele fractions specified in initial_alleles_frequencies must add up to 1.0, not %v", alleleTotal)
	}
	return
}
...
func GenerateVariableFreqInitialAlleles(p *Population, uniformRandom *rand.Rand) {
	...
	for _, fracFreq := range freqList {
		...
		if numIndivs > int(p.GetCurrentSize()) {
			if numIndivs > int(p.GetCurrentSize()+1) {
				log.Printf("Internal Error: numIndivs of %d is > than population size", numIndivs)
			}
		...
		for i := 1; i <= numAlleles; i++ {
			...
			if !allIndivs {
				indivIndices = uniformRandom.Perm(int(p.GetCurrentSize()))
			} // as optimization don't mix pop order if we are going to give the allele to everyone anyway
...
func (p *Population) Mate(newP *Population, uniformRandom *rand.Rand) {
	...
	for i := range newP.Parts {
		...
		if segmentStart <= highestIndex {
			...
			if numMuts <= 100 {
				numMuts = numMuts * 2
			} // with small number the randomness of Poisson distribution can vary more
...
func (p *Population) IsDone(doLog bool) bool {
	...
	if popMaxIsSet && p.GetCurrentSize() >= popMax {
		if doLog {
			log.Printf("Tribe %d has reached the max specified value of %d. Stopping this tribe.", p.TribeNum, popMax)
		}
		return true
		...
		if doLog {
			log.Printf("Tribe %d is extinct. Stopping this tribe.", p.TribeNum)
		}
		return true
		...
		if doLog {
			log.Printf("Tribe %d fitness is below the extinction threshold of %.3f. Stopping this tribe.", p.TribeNum, config.Cfg.Computation.Extinction_threshold)
		}
		return true
	}
	return false
...
func ParseMultipleBottlenecks(bottlenecksStr string) *Bottlenecks {
	...
	if strings.TrimSpace(bottlenecksStr) == "" {
		log.Fatal(errorStr)
	}
	...
	for i, t := range tuples {
		...
		if err != nil {
			log.Fatalf("Error parsing growth-rate in element %d of multiple_bottlenecks: %v", i+1, err)
		}
		if bottle.GrowthRate <= 0.0 {
			log.Fatalf("Error: growth-rate in multiple_bottlenecks must be > 0.0, not %v", bottle.GrowthRate)
		}
...
func (p *Population) GetFitnessStats() (float64, float64, float64, uint64, float64) {
	...
	for _, indRef := range p.IndivRefs {
		...
		if ind.GenoFitness > p.MaxFitness {
			p.MaxFitness = ind.GenoFitness
		}
		if ind.GenoFitness < p.MinFitness {
			p.MinFitness = ind.GenoFitness
		}
...
func (p *Population) getAlleles(genNum, popSize uint32, lastGen bool) (alleles *dna.AlleleCount) {
	...
	for i := range p.IndivRefs {
		...
		if i != 0 && gcInterval > 0 && (i%int(gcInterval)) == 0 {
			config.Verbose(1, "Counted alleles in %d individuals", i)
		}
	}
	return
}
...
func (p *Population) outputAlleleBins(genNum, popSize uint32, lastGen bool, alleles *dna.AlleleCount) {
	...
	if deleterious > 0 {
		delFitness = (delDomFitness + delRecFitness) / float64(deleterious)
	}
	favorable := favorableDom + favorableRec
	if favorable > 0 {
		favFitness = (favDomFitness + favRecFitness) / float64(favorable)
	}
	if delAllele > 0 {
		delAlleleFitness = delAlleleFitness / float64(delAllele)
	}
	if favAllele > 0 {
		favAlleleFitness = favAlleleFitness / float64(favAllele)
	}
	config.Verbose(1, "---------")
	...
	if config.FMgr.IsDir(config.ALLELE_BINS_DIRECTORY) {
		...
		if err != nil {
			log.Fatalf("error marshaling allele bins to json: %v", err)
		}
...
func (p *Population) outputNormalizedAlleleBins(bucketJson *Buckets, bucketCount uint32, genNum uint32, fileName string) {
	...
	if err != nil {
		log.Fatalf("error marshaling normalized allele bins to json: %v", err)
	}
...
func fillBuckets(counts map[uint64]dna.Allele, popSize uint32, bucketCount uint32, buckets []uint32) (totalMutns uint64, totalFitness float64) {
	...
	if !config.Cfg.Computation.Count_duplicate_alleles {
		poolSize = float64(popSize)
	} // in this case, each allele count is a measure of how many individuals it occurred in
...
func (p *Population) sortIndexByPhenoFitness() {
	...
	if config.IsVerbose(9) {
		...
		for i, ind := range p.IndivRefs {
			fitSlice[i] = ind.Indiv.PhenoFitness
		}
...
func (p *Population) outputAlleleDistribution(genNum, popSize uint32, lastGen bool, alleles *dna.AlleleCount) {
	if !(config.FMgr.IsDir(config.DISTRIBUTION_DEL_DIRECTORY) || config.FMgr.IsDir(config.DISTRIBUTION_FAV_DIRECTORY)) {
		return
	}
	...
	*/
	...
	if x == 0 {
		x = 1.
	} // don't scale data if fraction_neutral = 1
	...
	if alleleWriter := config.FMgr.GetDirFile(config.DISTRIBUTION_DEL_DIRECTORY, delFileName, p.TribeNum); alleleWriter != nil {
		...
		newJson, err := json.Marshal(bucketJson)
		if err != nil {
			log.Fatalf("error marshaling allele distribution bins to json: %v", err)
		}
		if _, err := alleleWriter.Write(newJson); err != nil {
			log.Fatalf("error writing alleles to %v: %v", delFileName, err)
		}
	}
	if alleleWriter := config.FMgr.GetDirFile(config.DISTRIBUTION_FAV_DIRECTORY, favFileName, p.TribeNum); alleleWriter != nil {
		...
		if _, err := alleleWriter.Write(newJson); err != nil {
			log.Fatalf("error writing alleles to %v: %v", favFileName, err)
		}
	}
